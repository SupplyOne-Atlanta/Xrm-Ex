            ...
            async update(data) {
                try {
                    if (!this.Id || !this.EntityType || !data)
                        return null;
                    const record = await Xrm.WebApi.updateRecord(this.EntityType, this.Id, data);
                    return record;
                }
                catch (error) {
                    throw new Error(`XrmEx.${XrmEx.getFunctionName()}:\n${error.message}`);
